# ============================================================
# GRU REGRESSOR - INDEPENDENT PIPELINE
# ============================================================
...
# ------------------------------------------------------------
# 1. LOAD DATA & GENERATE 3D SEQUENCES
# ------------------------------------------------------------
...
# Normalización basada exclusivamente en train (<=2016)
...
print(f"\n📊 Resultados GRU -> MAE: {mae:.3f} | RMSE: {rmse:.3f} | R2: {r2:.3f}")

# ------------------------------------------------------------
# 4. EXPORT SUMMARY & PREDICTIONS
# ------------------------------------------------------------
summary_df = pd.DataFrame([{"Model": "GRU", "MAE": round(mae, 4), "RMSE": round(rmse, 4), "R2": round(r2, 4)}])
summary_df.to_excel("gru_summary.xlsx", index=False)
summary_df.to_csv("gru_summary.csv", index=False)
...
pred_df.to_csv("predictions_gru.csv", index=False)

print("\n✅ Archivos guardados: gru_summary.xlsx, gru_summary.csv, predictions_gru.csv")
