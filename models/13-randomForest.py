# ============================================================
# RANDOM FOREST REGRESSOR - INDEPENDENT PIPELINE
# ============================================================
...
print(f"\n📊 Resultados Random Forest -> MAE: {mae:.3f} | RMSE: {rmse:.3f} | R2: {r2:.3f}")

# ------------------------------------------------------------
# 3. EXPORT SUMMARY & PREDICTIONS
# ------------------------------------------------------------
summary_df = pd.DataFrame([{"Model": "Random Forest", "MAE": round(mae, 4), "RMSE": round(rmse, 4), "R2": round(r2, 4)}])
summary_df.to_excel("rf_summary.xlsx", index=False)
summary_df.to_csv("rf_summary.csv", index=False)
...
print("\n✅ Archivos guardados: rf_summary.xlsx, rf_summary.csv, predictions_rf.csv")