# ============================================================
# TEMPORAL TRANSFORMER - INDEPENDENT PIPELINE
# ============================================================
...
# ------------------------------------------------------------
# 1. LOAD DATA & BUILD TRANSFORMER SEQUENCES
# ------------------------------------------------------------
...
def build_transformer(input_shape):
    ...
    ff_output = layers.Dense(32, activation="relu")(x)
...
print("\n--- Entrenando Temporal Transformer ---")
...
print(f"\n📊 Resultados Transformer -> MAE: {mae:.3f} | RMSE: {rmse:.3f} | R2: {r2:.3f}")

# ------------------------------------------------------------
# 3. EXPORT SUMMARY & PREDICTIONS
# ------------------------------------------------------------
summary_df = pd.DataFrame([{"Model": "Transformer", "MAE": round(mae, 4), "RMSE": round(rmse, 4), "R2": round(r2, 4)}])
summary_df.to_excel("transformer_summary.xlsx", index=False)
summary_df.to_csv("transformer_summary.csv", index=False)
...
pred_df.to_csv("predictions_transformer.csv", index=False)

print("\n✅ Archivos guardados: transformer_summary.xlsx, transformer_summary.csv, predictions_transformer.csv")